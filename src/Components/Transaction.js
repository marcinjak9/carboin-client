...
const Transaction = ({
  ...
  return (
    <CardStyled selected={selected} onClick={() => select(id)}>
      ...
      <div className="body">
        <ul className="impact">
          <li>CO2 Produced: <span>{co2produced}</span></li>
          <li>CO2 Regenerated: <span>{co2regenerated ? "280 kg" : "0 kg"}</span></li>
          <li>Carbon cost: <span>{co2regenerated ? '0 $' : '2.8 $'}</span></li>
        </ul>
        <div className="action">
          Click to add to decarbonization
        </div>
      </div>
    </CardStyled>
  )
